...
<article class="[&:not(:last-child)]:border-b border-gray-100 pb-10">
    <div class="article-body grid grid-cols-12 gap-3 mt-5 items-start">
        ...
        <div class="col-span-8 flex flex-col justify-between px-4 gap-4">
            <div class="article-meta flex py-1 text-sm items-center">
                <img class="w-7 h-7 rounded-full mr-3" src="{{ $post->author->profile_photo_url }}"
                    alt="{{ $post->author->name }}">
                <span class="mr-1 text-xs dark:text-gray-200">{{ $post->author->name }}</span>
            ...
            <div id="article-body">
                ...
                <p class="mt-2 text-base text-gray-700 font-light dark:text-gray-200">
                    {{ $post->getExcerpt() }}
                </p>
            </div>
            <div>
                ...
                <div class="article-actions-bar mt-4 flex items-center justify-between">
                    <div class="flex items-center space-x-4">
                        <span class="text-gray-500 text-sm dark:text-gray-400">{{ $post->getReadingTime() }} min
                            read</span>
                    </div>
                    <div>
